...
const CountdownTable = () => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const response = await axios.delete(`http://localhost:5000/api/countdown/deletecountdown/${id}`);
  ...
  return (
    ...
    >
      ...
      >
        <h1 className="fw-bold">Countdown Table</h1>
        <p className="text-muted">Your Countdown list is here</p>
      </div>
      ...
      >
        <div className="d-flex justify-content-between align-items-center">
          <h3 className="text-center mb-4 mx-auto">Counter List</h3>
          <button className="btn btn-primary">
            <Link to={"/admin/countdown/addcountdown"} style={{ color: "white" }}>
              Add Counter
            </Link>
          </button>
        </div>
